...
## 10. Logo produit (reviews) + workflow tout-overlay

Pour une **review d'un produit nommé**, ajouter le **vrai logo** de l'outil sur la miniature
booste le CTR : reconnaissance instantanée par l'audience qui cherche ce produit.

- **Logo = asset réel, jamais généré DE MÉMOIRE par IA** (sinon baragouin). Téléchargé depuis le
  site / press kit de la marque. Outil de composition libre : Canva, OU ChatGPT en mode édition
  d'image — dans ce cas UPLOADER le PNG du logo comme référence et demander de l'incruster
  (il le pose tel quel, ne l'invente pas). Le texte court (ONE FLAG / TEN ACCOUNTS) peut aussi
  être posé par ChatGPT — vérifier ortho + hex exact après coup.
- **Workflow propre = tout en overlay.** L'IA génère l'image SEULE (sphère/visuel, `NO text,
  no logos`). Dans Canva on pose : le texte (police + hex exacts) ET le logo. Un seul prompt
  image+texte (variante B) ne tient plus dès qu'on veut le logo réel → toujours séparé pour les
  reviews. Bonus : texte et logo swappables pour A/B test sans retoucher l'image.
- **Placement :** logo petit et propre (haut, ou près du texte), sans voler la vedette au point
  focal. Texte jamais dans les 20 % du bas.
- **Droits :** usage nominatif (montrer le produit qu'on critique) généralement OK ; comme on
  est affilié, vérifier les brand/affiliate guidelines de la marque et ne pas suggérer un
  partenariat/endorsement officiel.
